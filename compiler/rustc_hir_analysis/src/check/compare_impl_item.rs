...
fn compare_method_predicate_entailment<'tcx>(
    ...
    // When the impl provides a value for an associated trait (e.g.,
    // `trait Arg = IntoIterator<Item = i32>`), we expand any
    // `T: <ConcreteType as Trait>::AssocTrait` predicates in the
    // hybrid param_env into the concrete value trait bounds so that
    // the impl method body can use the expanded capabilities.
    {
