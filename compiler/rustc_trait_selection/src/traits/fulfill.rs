                        ...
                        match self.selcx.select(&trait_obligation) {
                            Ok(Some(ImplSource::UserDefined(impl_source))) => {
                                    ...
                                    specialization_graph::assoc_def(tcx, impl_def_id, assoc_item_id)
                                {
                                    ...
                                    // Expand the impl item's value bounds into
                                    // concrete trait + projection obligations.
                                    for clause in
                                        tcx.expand_assoc_trait_value(impl_item_def_id, pred.self_ty)
                                    {
                                        new_obligations.push(obligation.with(tcx, clause));
                                    }
